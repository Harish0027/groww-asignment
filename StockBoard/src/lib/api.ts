...
function aggregateToWeekly(data: StockHistoricalData[]) {
  const weekly: any[] = [];
  let week: any = null;

  data.forEach((day) => {
    const d = new Date(day.date);
    const weekNumber = `${d.getFullYear()}-${Math.floor(d.getDate() / 7)}`;
...
function aggregateToMonthly(data: StockHistoricalData[]) {
  const monthly: any[] = [];
  let month: any = null;

  data.forEach((day) => {
    const d = new Date(day.date);
    const monthNumber = `${d.getFullYear()}-${d.getMonth()}`;

    if (!month || month.month !== monthNumber) {
...
function normalizeHistorical(
  ...
  // Finnhub candles
  if (raw.c && raw.t) {
    return raw.t.map((ts: number, idx: number) => ({
      date: new Date(ts * 1000).toISOString().split("T")[0],
      open: raw.o ? raw.o[idx] : raw.c[idx],
      high: raw.h ? raw.h[idx] : raw.c[idx],
      low: raw.l ? raw.l[idx] : raw.c[idx],
      close: raw.c[idx],
      volume: raw.v ? raw.v[idx] : 0,
    }));
  }

  // Alpha Vantage (Daily / Weekly / Monthly)
  if (raw["Time Series (Daily)"]) {
    return Object.entries(raw["Time Series (Daily)"]).map(
      ([date, val]: any) => ({
        date,
  ...
  if (raw.historical) {
    return raw.historical.map((row: any) => ({
      date: row.date,
      open: row.open,
      high: row.high,
      low: row.low,
...
export async function getHistoricalData(
  ...
  try {
    ...
    return normalizeHistorical(symbol, interval, data); // ALWAYS StockHistoricalData[]
  } catch (error) {
